...
/* FIX 1 — openMobileNav / closeMobileNav were called in HTML but not defined */
...
function setModel(m, btn) {
  activeModel = m;
  document.querySelectorAll('.tab-btn, .seg-btn').forEach(b => b.classList.remove('active', 'active-seg'));
  if (btn) { btn.classList.add('active'); btn.classList.add('active-seg'); }
  const names = {
    ...
    nn2: 'Custom Built Model',   /* FIX 6 — was missing, caused silent failure on URL share */
  };
